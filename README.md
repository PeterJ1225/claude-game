...
**进度：M0–M8 全部里程碑完成**（脚手架 → 桌面壳/存储 → 农场循环 → 经济小镇 → 季节种植 → 社交 → 矿洞战斗 → 钓鱼 → 节日 → 打磨）。核心闭环可玩：种植/养成、四季天气、买卖经济、NPC 社交与心事件、采矿战斗、钓鱼、节日，含存读档与设置菜单。下方按里程碑列出各自增量与验证。
...
已实现：
- **设置菜单**（`ESC` 打开，暂停游戏与时间）：音乐/音效音量增减、全屏切换、语言（中文，预留 i18n）、键位说明；写入 `settings` 随存档保存。
- **程序化音频**：`AudioManager` 用 Web Audio API 即时合成音效（收获、得金、升级、新的一天），受音效音量控制；无二进制音频文件（真实 CC0 音轨待整合，见 `public/assets/CREDITS.md`）。任何不支持/异常一律静默不崩。
- **性能/无泄漏**：场景在 `SHUTDOWN` 时注销所有 EventBus 订阅；单测验证连续过夜 10 天订阅数不增长。
- **UI/文案润色**：主菜单与提示更新；设置内集中键位说明。
- **数值平衡**：以附录 A 基线为准做了一轮通读，保留为可玩基线（改动会登记 ADR）。
- **打包**：见下「构建与打包」。

> 美术仍为运行时生成的占位（M0 起），音频为程序化合成；真实 CC0 素材整合是后续工作，来源/许可证登记在 `public/assets/CREDITS.md`。

验证：`tsc` / `eslint` / `vitest`（**77 测试**，含设置 clamp/写入、过夜 10 天无监听器泄漏）/ `vite build` 全绿。

## 构建与打包

- **Web（静态站点）**：`npm run build` → 产物在 `dist/`，可直接部署到任意静态托管（已配 `base:'./'` 相对路径）。`npm run preview` 本地预览。
- **桌面（Tauri 2）**：`npm run tauri:build` 产出 Windows 安装包（在 `src-tauri/target/release/bundle/`）。**需先装 Rust 工具链 + Windows Build Tools + WebView2 Runtime**；`npm run tauri:dev` 为桌面开发窗口。脚本在缺 Rust 时给出前置提示。
  > ⚠️ 本机未装 Rust，故 `tauri:build`/`tauri:dev` 这两步未在此环境验证；装好工具链后即可运行。浏览器构建（`npm run build`）不受影响、已验证通过。
